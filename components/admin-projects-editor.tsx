...
export default function AdminProjectsEditor({ initialItems, dbReady }: { initialItems: ProjectItem[]; dbReady: boolean }) {
  const router = useRouter();
  const sortedInitial = useMemo(() => [...initialItems].sort((a, b) => a.sortOrder - b.sortOrder || a.id - b.id), [initialItems]);
  const [items, setItems] = useState<ProjectItem[]>(sortedInitial);
  const [baselineItems, setBaselineItems] = useState<ProjectItem[]>(sortedInitial);
  const itemsRef = useRef<ProjectItem[]>(sortedInitial);
  ...
  async function persistOrder(nextItems: ProjectItem[]) {
    setOrderError(null);
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                onDragStart={(e) => {
                  ...
                  const row = (e.currentTarget as HTMLElement).closest('[data-project-row="1"]') as HTMLElement | null;
                  if (row) {
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-gray-600 dark:text-white/60 truncate">{item.linkUrl}</div>
            </div>
