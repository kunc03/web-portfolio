...
export default function AdminMenuEditor({ initialItems, dbReady }: { initialItems: MenuItem[]; dbReady: boolean }) {
  const sortedInitial = useMemo(() => [...initialItems].sort((a, b) => a.sortOrder - b.sortOrder || a.id - b.id), [initialItems]);
  const [items, setItems] = useState<MenuItem[]>(sortedInitial);
  const [baselineItems, setBaselineItems] = useState<MenuItem[]>(sortedInitial);
  const itemsRef = useRef<MenuItem[]>(sortedInitial);
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                onDragStart={(e) => {
                  ...
                  const row = (e.currentTarget as HTMLElement).closest('[data-menu-row="1"]') as HTMLElement | null;
                  if (row) {
